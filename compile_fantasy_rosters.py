...
expected_columns = ['Fantasy Owner','Fantasy Owner Email','Fantasy Team Name',
                    'Automation Mapping','Roster','Position Type','Position Code',
                    'Position Group','Team Abbr.','Selection','Check 1 - Selection is Unique',
                    'Check 2 - Team is Unique']
minimum_positions = ['K','QB1','QB2','QB3','RB1','RB2','RB3','TE1','TE2','WR1','WR2','WR3','D']
...
for file in files:
  if file.endswith('.csv') and 'Playoff Fantasy Roster 2024' in file:
    ...
    assert(len(tmp_df.columns)==12) # check that the roster has correct number columns
    assert(all(x in expected_columns for x in tmp_df.columns.values)) # check that the column names match
    assert(len(tmp_df)==14) # check that each roster is correct length
    assert(tmp_df['Automation Mapping'].is_unique) # check all automation mapping is unique
    assert(not tmp_df['Fantasy Team Name'].is_unique) # check all team names are the same
    assert(tmp_teamname not in team_names) # check that no other team has the same team name
    if tmp_teamname not in team_names: 
      team_names.append(tmp_df['Fantasy Team Name'][1])
    assert(set(minimum_positions).issubset(tmp_positions)) # check minimum positions are present
    ...
    for p in tmp_positions:
      if p in minimum_positions:
        ...
        tmp_counts.append(p)
    assert(set(tmp_counts).issubset(extra_positions)) # check that there is an extra position
    assert(len(tmp_counts)==1) # check that there is only one extra position (should pass if previous checks passed)
    assert(not df.isnull().values.any()) # check that there are no NaNs in the dataset
...
df.drop(columns=['Fantasy Owner Email','Position Group','Selection','Check 1 - Selection is Unique','Check 2 - Team is Unique'], inplace=True)
df.head()
